...
class PacienteRepository(IPacienteRepository):
    ...
    def crear_paciente(self, paciente: Paciente) -> Paciente:
        ...
        try:
            ...
            try:
                ...
                raise SGCMConflictError("El número de identificación ya existe en el sistema")
        finally:
            cursor.close()
            conexion.close()
    ...
    def reemplazar_alergias(self, id_paciente_fk: int, alergias: list[str]):
        ...
        try:
            cursor = conexion.cursor()
            cursor.execute(
                "DELETE FROM paciente_alergias WHERE id_paciente_fk = %s",
                (id_paciente_fk,)
            )
            ...
            conexion.commit()
            return self.obtener_por_id(id_paciente_fk)
        finally:
            cursor.close()
            conexion.close()
